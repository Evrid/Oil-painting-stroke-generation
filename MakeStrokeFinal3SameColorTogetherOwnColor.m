...
%if you don't want draw step by step then comment line 345 drawnow; by
%adding a %
...
maxStrokeLength = 1;   %the larger the less exquisite  was 1
minStrokeLength = 0.5;   %the larger the less exquisite  was 0.5
...
for ooo=1:size(SumOfStrokeInColorOrder,1)
    ...
    RGBForEachStroke=[SumOfStrokeInColorOrder(ooo,5) SumOfStrokeInColorOrder(ooo,6) SumOfStrokeInColorOrder(ooo,7)];
    %above we draw use their own color, not standerdized color (if so then we first detect which color is needed in the picture then mix them ourself)  
